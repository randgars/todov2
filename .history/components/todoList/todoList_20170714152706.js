...
export default class TodoList extends React.Component {
  ...
  getSalectedDateEvents() {
    
    for (let i = 0; i < this.props.list.length; i++) {
      if (this.props.screenProps.datesEventsList[i].date == this.props.selectedDate) {
        debugger
        this.setState({
  ...
  render() {
    return (
      <View style={styles.listContainer}>
        <FlatList
          data={ this.props.list && this.props.list.map((item) => ({ key: item.event})) }
